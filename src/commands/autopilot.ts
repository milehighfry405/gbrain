 ...
 *   gbrain autopilot --install [--repo <path>] [--env-file <path>]
...
// workspace-l5o.34: pure exported template so unit tests assert the env-source
// order, --env-file precedence, and shell-injection escaping without writing
// to disk. When `envFile` is set, source it FIRST (set -a so the assignments
// export to children), then layer shell-rc on top — operators can still
// override via ~/.zshenv. Use when API keys live in a host/app env file
// outside the shell-rc convention (AlphaClaw containers store secrets in
// /data/.openclaw/.env; default install couldn't reach them, autopilot
// subagents permanently failed with "Could not resolve authentication method").
export function generateWrapperScriptContent(
  gbrainPath: string,
  repoPath: string,
  envFile?: string,
): string {
  ...
  const envFileBlock = envFile
    ? (() => {
        const safeEnvFile = envFile.replace(/'/g, "'\\''");
        // Codex P2#1 (this commit): the && chain skipped `set +a` when source
        // returned nonzero, leaving allexport enabled while ~/.zshenv etc were
        // sourced afterwards. The if/fi block always restores allexport.
        return `# --env-file: source host/app env file before shell rc so secrets in app-
# specific locations (e.g. AlphaClaw /data/.openclaw/.env) reach autopilot
# even when not exported via ~/.zshenv. set -a exports assignments to children.
if [ -f '${safeEnvFile}' ]; then
  set -a
  source '${safeEnvFile}' 2>/dev/null || true
  set +a
fi
`;
      })()
...
${envFileBlock}[ -f ~/.zshenv ] && source ~/.zshenv 2>/dev/null
source ~/.zshrc 2>/dev/null || source ~/.bashrc 2>/dev/null || true
exec '${safeGbrainPath}' autopilot --repo '${safeRepoPath}'
`;
}

function writeWrapperScript(repoPath: string, envFile?: string): string {
  const home = process.env.HOME || '';
  ...
  const wrapper = generateWrapperScriptContent(gbrainPath, repoPath, envFile);
...
async function installDaemon(engine: BrainEngine, args: string[]) {
  ...
  // workspace-l5o.34: --env-file lets the operator point the wrapper at a
  // host/app env file (e.g. /data/.openclaw/.env on AlphaClaw) so API keys
  // reach the autopilot subprocess even when not exported via ~/.zshenv.
  // Codex P2#2 (this commit): resolve the path at install time so a relative
  // arg doesn't silently miss when the daemon runs from a different cwd
  // (launchd/nohup/systemd don't inherit the user's install-time cwd).
  const envFileArg = parseArg(args, '--env-file');
  const envFile = envFileArg ? resolvePath(envFileArg) : undefined;

  const wrapperPath = writeWrapperScript(repoPath, envFile);
  const home = process.env.HOME || '';
